...
核心功能全部可用，测试见下文基线（唯一没绿的是裁剪的纵向朝向，已知未解决），
...
crop-browser      14   裁剪几何（13 通过 + 1 **已知问题**：纵向朝向）
...
6. **⚠️ `contextBridge` 暴露的对象是冻结的，替换不了。**
...
1. **⚠️ 先把裁剪的纵向朝向解决掉** —— 半成品状态，见 `ROADMAP.md` §5.1.1。
   那里记了已排查掉的假设、当前关键数值，和**建议的下一步排查方法**
   （别再推理，直接做「输出位置 → 采样坐标」的实测对照表）。
   **交付真人使用前必须解决**（纵向构图会被弄反）。
2. **继续做本地修图工具** —— 已完成锐化/暗角/颗粒/曲线/HSL/裁剪旋转，
   还剩**液化 / 透视校正**（高，需要网格变形或独立 shader pass）。
   这些是**零成本、实时、照片不出设备**的，比云 AI 更值得日常用。
3. 局部调整增强（渐变蒙版、径向蒙版）—— 复用现有蒙版引擎，只是换个填法
4. 修图结果回存相册（内容寻址天然支持非破坏性）
5. 火山任务持久化（现在关掉 App 任务就丢）
6. **密钥轮换** —— 之前在对话里贴过明文密钥，建议去控制台换一遍，
