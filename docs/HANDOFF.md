...
|---|---|
| 锐化 / 暗角 / 颗粒 / 曲线 / HSL | ✅ |
| 裁剪 / 旋转 | ✅ 完成（含一次查了很久的排查记录，见 ROADMAP §5.1.1） |
| 液化 / 透视校正 | 待做 |

⭐ **裁剪的「纵向朝向」已经真修好了**（原来登记为已知问题）。

根因不是一个，是**三个叠在一起**：`offY` 公式按框中心算了（应该锚定框的边、
且用钳制后的缩放）、`applyCrop` 的行序翻转被误删过、
`cropRenderPlan` 没考虑输出比例被钳制时的采样区间。
详细排查过程和**方法论教训**写在 ROADMAP §5.1.1 —— 值得一读，
因为最后管用的方法不是推理，是"造位置编码图 + 实测对照表"。

守它的测试分两层，缺一不可：

- `test/crop-geometry.test.mjs`（15 项，源码级，快）：从 studio.js 里
  **抠出公式求值**，断言采样区间精确等于取景框那块原图。
  ⚠️ 这层是必须的 —— `crop-browser` 对 `offY` 的符号是**盲的**
  （它读的那几个点两种公式颜色碰巧一样）。
- `test/crop-browser.test.mjs`（16 项，真 WebGL）：用**行编码图**量实际
  采到的源行；预览与烘焙的朝向；90° 整转的四象限颜色。
...
云 AI 三家都接完了，本地工具也做了七个
（锐化/暗角/颗粒/曲线/HSL/裁剪/旋转）。再往上加：

- **液化 / 透视校正**（高）—— 需要网格变形或独立 shader pass
  （现在的 shader 是**单趟**的，液化要多趟，成本主要在这里）
...
>
> ⚠️ 还有一类（裁剪那种）：**"算出来的数"错了**。这类要的是
> **源码级区间等式**（`test/crop-geometry.test.mjs` 那种，
> 把公式从源码里抠出来求值，和几何期望精确比对）——
> 颜色判据太粗，抓不住"偏了半个框"这种错误。
...
| `tools/gen-assets.mjs` | xiaobubuya.github.io | 生成品牌素材（图标/分享封面/播放页视觉），调 Agnes |
...
✅ 能用：上传、相册排版、翻页阅读、分享、WebGL 调色、
        锐化/暗角/颗粒/曲线/HSL、**裁剪/旋转**、蒙版局部调整、
        AI 抠人、AI 去物、AI 美颜（**按原图分辨率送**）、
        一键人像模板（8 个）、左右分屏对比、AI 进度条、导出
📋 没做：液化/透视校正、渐变与径向蒙版、
        修图结果回存、火山任务持久化
🔑 密钥：已全部迁到保险箱，本地文件已删
🧪 测试：见上表（前端 284 + 桌面 69 + 后端 212 + 两组 Electron 实测 13）
