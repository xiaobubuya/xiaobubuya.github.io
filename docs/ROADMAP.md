...
实现：mask.js 增加 `kind: 'gradient' | 'radial'` 两种 stroke 类型，
复用 `_paint()` 分发和 `begin/extend/end` 交互。渐变用 `createLinearGradient`
画渐变轴（起点白→终点透明），径向用 `createRadialGradient` + `scale`
画椭圆渐变。快捷键 G/R，三工具互斥切换。

#### ⚠️ "画笔涂抹没有什么反应" —— 不是蒙版算错了，是**用户看不见**
...
**先排除掉的可能**（探针实测，都是对的）：
涂一笔 → 覆盖率 1.2%、包围盒正好落在涂的位置、蒙版画布中心 255 边缘 0、
边缘是干净的硬边。**蒙版引擎没问题**，左下角也一直在报"已选 x% · n 笔"。
所以这不是"画不上"，是"看不见"。

**真正的原因（两个）**

1. **"显示选区"默认关着 → 进画笔工具后涂出来画面一片空白。**
   用户看不到任何反馈，自然判断成"工具坏了"；文字统计更新了他也不会往那看。
   → 修：点画笔/渐变/径向时**自动打开**"显示选区"。
   用 `userHidMask` 记住"用户自己关过"，关过之后不再强行打开 ——
   他关掉是在说"我知道我在涂什么，别挡着我看照片"。

2. **红罩太淡 + 羽化太宽 = "一片看不出边界的渐变"。**
   原来不透明度只有 0.45；而画笔默认硬度 0.5，实心区只占半径一半，
   剩下全是过渡带。两点叠加就是用户看到的那片渐变。
   → 修：不透明度提到 0.62 并往暖黄偏一点
   （只叠纯红在**深色衣服**上几乎看不出来 —— 红罩在暗部不动）；
   另外**沿选区边界描一条亮边**：0→1 的过渡带本身没有边界可言，
   但它的**梯度峰值**正是用户心里的"选区边"，用 3×3 采样求 |∇m|，
   在峰值处叠亮色。这是让软边选区可读的关键。

**顺带抓到的真 bug：画笔光标圈的基准搞错了**
`showCursor` 用 `mask.radius * min(画布宽, 画布高) * 2`，
而真正涂抹（mask.js 的 `_px`）用 `radius * min(图片宽, 图片高)` ——
**两者基准不同**。画布和图片恰好同比例时数值相同（757×568 的画布配
400×300 的图都是 1.333），所以本地极难发现；舞台比图片更扁时圈就偏小，
用户会"明明涂在圈里却涂到旁边"。
实测把画布拉成 894×300：正确 53.6px，旧写法只有 24.0px（小一半多）。

⚠️ **这个 bug 能测出来，靠的是"把画布拉成非图片比例"**。第一版测试没拉
画布，两种算法恰好相等 —— 测了等于没测。`test/mask-ui-browser.test.mjs`
那条专门把画布设成 894×300 再量，并用"把公式换成旧写法"的变异验证过：
旧写法会红（24.0 vs 53.6）。

**另一个只在测试里出现的坑**：`Studio.paint()` 这个测试辅助调
`mask.begin(x, y)` 时**没传工具参数**，而 `begin` 不传时用的是"上一次的
工具" —— 在画笔模式下点过渐变按钮之后它还是 gradient，于是"涂一笔"
实际画出来的是**渐变**。表现和"蒙版纹理传坏了"一模一样，查了不少时间。
改成默认 `'brush'`。

**测试**：`test/mask-ui-browser.test.mjs`（6 项）——
进工具自动显示选区、涂过的地方真的变红（读像素）、没涂的地方不变红、
光标圈大小与笔下范围一致、画布拉成非图片比例时圈不缩水、擦除模式换样式。

### 5.3 修图页 UI 升级（参考像素蛋糕）

当前右侧面板"能用但不好用"：没有预设系统、section header 只有文字、AI 模板只有文字按钮。
参考像素蛋糕桌面版 UI，升级三个方向。详见 `docs/PRD-UI.md`。

| 子项 | 难度 | 说明 | 状态 |
|---|---|---|---|
| 预设系统 | 中 | 14 个分类预设（婚纱/写真/旅拍/胶片/黑白/日常），色块预览 + 一键应用 | 待做 |
| 面板视觉升级 | 低 | Section header 加图标 + 折叠动画 + 配色微调 | 待做 |
| 模板缩略图 | 低 | AI 模板从文字按钮升级为缩略图卡片网格 | 待做 |
