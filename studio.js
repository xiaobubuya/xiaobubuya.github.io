...
(function () {
  ...
  function cropRenderPlan(forExport = false) {
    ...
    /* ⚠️ 采样缩放：x / y **分别算**。这里错过两次，把两次都留下：
       ================================================================
       【错法一】`s = ins.w / box.W` 一个标量同时管 x 和 y。
       取景框等于整个内接矩形时恰好蒙对，但用户一把框拖小，
       采样区间就被**放大**了。

       【错法二】改成 `sX = r.w*ins.w/W0; sY = r.h*ins.h/H0` 之后，
       仍隐含一个假设：**取景框的宽高比 == 输出的宽高比**。
       两者不等时画面会被**拉长** —— 因为输出画布用的是框的比例，
       而"框比内接矩形允许的最宽值还宽"时输出会被钳制
       （applyCropAspect 和 setCropRect 两个入口都可能留下这种框）。

       实测反例：400×300 的图，rect = {x:0, y:0, w:1, h:0.5}（2:1），
       再设成 1:1 → 输出被钳到 400×400，但采样区间还按 2:1 算，
       纵向只采一半 → 画面纵向被拉长一倍。
       crop-geometry.test.mjs 里"采样区间的宽高比必须等于输出宽高比"
       那条就是钉它的。

       正确做法：先算"按目标比例允许的最大框"，再往里缩，用缩完的
       尺寸当采样区间：
         图上采样宽 = min(r.w·insW, (r.h·insH)·R)      R = 目标宽高比
         图上采样高 = 图上采样宽 / R
       这样"采样区间宽高比"恒等于"输出画布宽高比"，
       而输出尺寸公式 `viewW·insW/box.W` 用的是同一组钳制，两边对齐。

       x / y 必须分开：旋转是在"外接矩形"里做的，
       两轴的比例因子不同，共用一个必然有一边错。 */
    const R = (r.w * ins.w) / (r.h * ins.h);              // 目标宽高比
    const apW = Math.min(r.w * ins.w, r.h * ins.h * R);   // 图上采样宽
    const apH = apW / R;
    const sX = apW / W0;
    const sY = apH / H0;

    /* 偏移：把采样区间摆到取景框对应的那块位置上。
       ================================================================
       ⚠️⚠️ 这是整个裁剪功能里最难的一处，靠推理**收敛不了** ——
       把推导记下来，免得下一个人再绕一遍。

       两个坐标系的 y 方向**相反**（这是全部麻烦的来源）：
         · `crop.rect` 是**屏幕约定**：y = 0 在画面**下**边、y = 1 在上边。
           证据是 cropRectOnCanvas()：`r.y = 1` 时框画在画布顶部。
         · shader 采样的 `u.y` 是**图片约定**：0 = 图**上**边
           （VERT 里 `vUv.y = 0.5 - aPos.y*0.5`，画面顶部 vUv.y = 0）。

       ⚠️ 还有第二层：上面算 sX/sY 时**采样区间被比例钳制过**
       （apW / apH 可能小于框本身的尺寸）。钳制后区间变小了，
       "往哪边缩"必须和用户看到的一致 —— 规则是**锚定在框的边上往里缩**：
         · 纵向被钳制（框太矮）→ 锚定框的**下边**
         · 横向被钳制（框太宽）→ 锚定框的**左边**
       居中缩会让取景框"自己漂走"，实测就是这个症状。

       以"锚定下边"为例：
         采样区间下缘（图片约定）= 0.5 + offY + sY/2
         框的下边（屏幕约定）    = r.y，换算到图片约定 = 1 - r.y
         令两者相等：0.5 + offY + sY/2 = 1 - r.y
                    offY = 0.5 - r.y - sY/2

       横向同理（屏幕 x 和图片 u 同向，锚左边）：
         0.5 + offX - sX/2 = r.x - insLeft
         offX = r.x - insLeft + sX/2 - 0.5

       ⚠️ 关键：公式里用的是**钳制后**的 sX / sY，不是 r.w / r.h。
       第一版用了 r.h/2 —— 框比例和输出比例一致时看不出问题，
       不一致时区间就整体偏了（实测「裁画面上半」采到 v∈[0.125,0.625]，
       正确是 v∈[0,0.5]，表现成"纵向朝向不对"，其实不是翻转）。

       定案靠 test/_probe-crop-map.mjs 的实测对照表：造一张
       "绿通道 = 图像行位置"的图，裁剪后扫输出画布上下两边，
       读出真实采样区间，和公式逐场景比对。
       ⚠️ 必须用 v2 —— v1 分好几轮跑，中间改过代码，
       跨轮次的数字混着看会得出自相矛盾的结论（实际绕了很久）。
       同样的规则在 test/crop-geometry.test.mjs 里有精确的源码级断言。 */
    const insLeft = (1 - ins.w / box.W) / 2;      // 内接矩形左边（旋转框归一化）
    const insBottom = (1 - ins.h / box.H) / 2;    // 内接矩形下边（屏幕约定，见下）
    const offX = r.x - insLeft + sX / 2 - 0.5;
    const offY = 0.5 - r.y - sY / 2;
    if (forExport) {
      return {
