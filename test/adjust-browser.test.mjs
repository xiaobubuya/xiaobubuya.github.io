...
import { launch, evaluate, shutdown, sleep } from './cdp.mjs';
...
async function t(name, body, check) {
  ...
  //
  // ⚠️ 每个用例都重新建标签页，所以都要先等页面 JS 跑完。
  // 一开始没等，前 4 个用例全挂（window.Studio 还是 undefined），
  // 后面的却过了 —— 这种"前面挂后面过"就是典型的竞态，不是功能坏了。
  const code = `(async () => {
    ${READY_WAIT}
    ${body}
  })()`;
  try {
    const r = await evaluate(chrome.port, URL, code);
...
try {
  ...
  `, r => {
    ...
    assert.equal(r.n, 9, `调整项应该是 9 个，实际 ${r.n}`);
    ...
    assert.equal(r.sliders, 9, `滑杆数应该和调整项一致，实际 ${r.sliders}`);
  });
